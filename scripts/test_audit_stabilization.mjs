...
test('mobile facilitator voice remains enabled, replayable, and independently audible on every authorized device', () => {
  ...
  assert.match(participantView, /Hear facilitator responses/);
  assert.match(participantView, /Enable sound/);
  assert.doesNotMatch(participantView, /Enable ElevenLabs audio/);
...
test('server-owned session start is immediate, singular, and does not block on a model response', () => {
  ...
  assert.match(startSessionSection[1], /async with _bounded_lifecycle_transaction\(\s*start_conn,\s*"start session",/);
