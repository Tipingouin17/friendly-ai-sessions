...
export const ProtectedHostRoute: React.FC<ProtectedHostRouteProps> = ({ children }) => {
  ...
  const [hostRouteError, setHostRouteError] = useState<'missing-session' | 'unauthorized' | 'check-failed' | null>(null);
    ...
    const checkHostStatus = async () => {
      ...
      try {
        ...
        if (!conversationId) {
          ...
          setHostRouteError('missing-session');
          setIsHost(false);
          setIsLoading(false);
          return;
        }
        ...
        if (error) {
          ...
          setHostRouteError('check-failed');
          setIsHost(false);
        } else {
          ...
          if (data) {
            ...
            setHostRouteError('unauthorized');
  ...
  if (!isLoading && !isHost) {
    const title = hostRouteError === 'missing-session'
      ? 'Missing host session'
      : hostRouteError === 'unauthorized'
      ? 'Host access unavailable'
      : 'Unable to verify host access';
    const description = hostRouteError === 'missing-session'
      ? 'This host link is missing the session ID. Please open the session from a workshop card or use a valid host link.'
      : hostRouteError === 'unauthorized'
      ? 'Your account is not listed as the host for this session. Please return to your facilitator dashboard and select an available workshop.'
      : 'We could not verify your host access for this session. Please return to your facilitator dashboard and try again.';
      ...
      <div className="flex min-h-screen items-center justify-center bg-background px-4">
        <div className="w-full max-w-md rounded-2xl border border-border bg-card p-8 text-center shadow-sm">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            <Button asChild>
              <Link to="/my-facilitators">Back to facilitators</Link>
            </Button>
            <Button asChild variant="outline">
              <Link to="/past-workshops">Past workshops</Link>
            </Button>
          </div>
        </div>
      </div>
