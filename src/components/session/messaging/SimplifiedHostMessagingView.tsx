...
const SimplifiedHostMessagingView: React.FC<SimplifiedHostMessagingViewProps> = ({
  ...
  const capabilitiesSummary = toolboxError
    ...
    : isLoadingToolbox
      ? 'Loading assigned facilitator tools…'
      : enabledToolCount > 0
        ? `${enabledToolCount} facilitator tool${enabledToolCount === 1 ? '' : 's'} assigned for background context.`
        : 'No additional facilitator tools are assigned for this session.';
              ...
              <div className="rounded-2xl border border-indigo-100 bg-gradient-to-br from-white via-indigo-50/50 to-violet-50/60 p-3 shadow-sm">
                <div className="flex flex-col gap-3">
                  ...
                  <div className="rounded-xl border border-slate-200 bg-white/75 px-3 py-2">
                    <p className="text-xs font-semibold text-slate-900">AI capabilities</p>
                    <p className="mt-1 text-[11px] leading-4 text-slate-500">Quiet context only; the host does not need to manage these during the live flow. {capabilitiesSummary}</p>
                  </div>
                </div>
              </div>
