...
export interface ParticipantSessionData {
  ...
  /** Browser-unique UUID generated by useDeviceId and stored in localStorage.
   *  Used to verify that a returning participant is using the same browser
   *  that originally joined — prevents participantId slot hijacking via URL. */
  deviceId?: string;
  timestamp: string;
...
/**
 * Look up participant data for a specific (conversationId, deviceId) pair.
 * Returns null if no matching entry exists in localStorage.
 * This is the primary lookup used by useSessionParticipantSetup to resolve
 * the current user's participantId without relying on fragile heuristics.
 */
export function readParticipantDataByDevice(
  conversationId: number,
  deviceId: string
  ...
  // participant goes through the join form and gets a fresh entry.
