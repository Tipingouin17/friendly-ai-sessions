...
export function useSessionContextValue({
  ...
  const safeRoomState = useMemo(() => roomState || {
    ...
    responseCount: 0,
    totalParticipants: 1,
    generateAggregatedResponse: async () => Promise.resolve(),
    isGeneratingResponse: false,
    forceFetchMessages: () => { /* no-op */ },
  ...
  const sessionState = useMemo(() => ({
    ...
    isWaitingForResponses: safeRoomState.isWaitingForResponses || false,
    responseCount: safeRoomState.responseCount || 0,
    totalParticipants: safeRoomState.totalParticipants || participants?.length || 1,
    generateAggregatedResponse: safeRoomState.generateAggregatedResponse || (async () => Promise.resolve()),
    isGeneratingResponse: safeRoomState.isGeneratingResponse || false,
    forceFetchMessages: safeRoomState.forceFetchMessages || (() => { /* no-op */ }),
