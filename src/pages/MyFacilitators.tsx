  ...
  return <div className="min-h-screen pt-24 pb-16 bg-[#FFC107]/10">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-white rounded-3xl shadow-lg p-8">
          ...
          <div className="mb-8">
            {currentStep === 1 && <FacilitatorSelection facilitators={facilitators} selectedFacilitator={selectedFacilitator} onSelect={setSelectedFacilitator} />}

            {currentStep === 2 && <WorkshopSelection workshops={workshops} selectedWorkshop={selectedWorkshop} onSelect={setSelectedWorkshop} />}

            {currentStep === 3 && <WorkshopSetup participantCount={participantCount} setParticipantCount={setParticipantCount} description={description} setDescription={setDescription} language={language} setLanguage={setLanguage} agreed={agreed} setAgreed={setAgreed} />}
          </div>

          <div className="flex justify-between">
            {currentStep > 1 ? <Button variant="outline" onClick={handlePrevious}>
                <ChevronLeft className="w-4 h-4 mr-2" /> Previous
              </Button> : <div></div>}
            {currentStep < 3 ? <Button onClick={handleNext}>
                Next <ChevronRight className="w-4 h-4 ml-2" />
              </Button> : <Button onClick={handleSubmit} disabled={!agreed}>
                Let's Begin!
              </Button>}
          </div>
        </div>
