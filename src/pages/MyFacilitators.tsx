...
const MyFacilitators = () => {
  ...
  return <div className="min-h-screen pt-16 bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {(hasReachedSessionLimit || hasReachedFacilitatorLimit) && <Alert variant="destructive" className="mb-6">
            <AlertCircle className="h-4 w-4" />
            <AlertTitle className="text-left">Plan Limit Reached</AlertTitle>
            <AlertDescription className="flex justify-between items-center">
              <span>
                {hasReachedSessionLimit ? `You've used ${currentSessionCount} out of ${maxSessions} sessions available in your plan.` : "You've reached your facilitator limit in your current plan."}
              </span>
...
const fetchFacilitators = async () => {
  const {
    data,
    error
  } = await supabase.from('facilitators').select('*').order('order', {
    ascending: true
  });
...
const fetchSessions = async (facilitatorId: number | null) => {
  const query = supabase.from('sessions').select('*, facilitator:facilitators!inner(*)').eq('status', true);
  if (facilitatorId) {
